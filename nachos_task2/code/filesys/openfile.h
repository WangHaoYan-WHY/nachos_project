...
class OpenFile {
  public:
    ...
    int Read(char *into, int numBytes, OpenFileId id); // Read/write bytes from the file,
  // starting at the implicit position.
  // Return the # actually read/written,
  // and increment position in file.
    int Write(char *from, int numBytes, OpenFileId id);
    ...
    void Seek(int position, std::string file_name);
  private:
    FileHeader *hdr;			// Header for this file 
    int seekPosition;			// Current position within the file
    int disk_sector;            // disk sector hdr
};
