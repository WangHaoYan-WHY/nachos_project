...
FileHeader::Allocate(PersistentBitmap *freeMap, int fileSize)
{
    ...
    std::cout << "--------------------------------------" << std::endl;
    std::cout << "use double indirect to initialize the file header" << std::endl;
    std::cout << "--------------------------------------" << std::endl;
    int allocated_num = 0;
    for(int i = 0; i < NumDirect && allocated_num < num_Sectors; i++) {
        d_block = new DoubleIndirect();
        if(dataSectors[i] == -1) {
          dataSectors[i] = freeMap->FindAndSet(); // find the free space
        }
        else {
            d_block->Fetch_From(dataSectors[i]); // fetch from the data sector
        }
        int res = d_block->Allocate_block(freeMap, num_Sectors - allocated_num); // allocate disk blocks
        d_block->Write_Back(dataSectors[i]); // write back
        allocated_num += res;
        delete d_block;
    }
    numBytes += fileSize;
