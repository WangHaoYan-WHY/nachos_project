          ...
          success = FALSE;    // no space in directory
            ...
            if (!hdr->Allocate(freeMap, initialSize)) {
              success = FALSE;    // no space on disk for data
                ...
                hdr->set_protection(protection); // set protection
...
FileSystem::new_Open(char *name, int mode, int work_dir_sector)
{
  ...
  if (kernel->removed_flag->find(current_path) != kernel->removed_flag->end() && (*kernel->removed_flag)[current_path] == TRUE) {
    printf("The path: %s is not removed");
    return -1;
  }
    Directory *directory = new Directory(NumDirEntries);
    ...
    OpenFileId res = kernel->currentThread->space->p_table->Insert(openFile, name, work_sector);
    delete directory; delete dir_File;
...
FileSystem::Open(char *name, int mode)
{
  ...
  if (sector >= 0) {
    ...
    if (kernel->removed_flag->find(current_path) == kernel->removed_flag->end()) {
      (*kernel->removed_flag)[current_path] = TRUE;
    }
    if ((fileHdr->get_protection() & mode) != mode) {
      printf("The protection permission is not permited");
      return NULL;
...
{
  if (kernel->currentThread->space->name_id.find(name) == kernel->currentThread->space->name_id.end()) {
    return FALSE;
  }
  kernel->currentThread->space->p_table->remove(kernel->currentThread->space->name_id[name]);
  return TRUE;
}
//----------------------------------------------------------------------
// FileSystem::Remove
