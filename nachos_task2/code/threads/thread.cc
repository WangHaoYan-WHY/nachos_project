...
{
  child_threads_ids = new List<ThreadId>();
  child_threads = new List<Thread*>();
  finished_childs = new List<ThreadId>();
    thread_id = kernel->thread_ids->FindAndSet();
    name = threadName;
    stackTop = NULL;
    stack = NULL;
    status = JUST_CREATED;
...
void Thread::set_Register(int num, int addr) {
  userRegisters[num] = addr;
}